...
/* $sql  = "SELECT @rownum := @rownum + 1 AS urutan,t.*
	     FROM staff t,
	    (SELECT @rownum := 0) r"; */
$sql = "SELECT attendance.id, attendance.date, staff.name, attendance.checkin, attendance.checkout, attendance.working_hours  FROM attendance INNER JOIN staff ON attendance.staff_id = staff.id";
